...
public class HueEffectWrapper {
	...
	private HueEffect handleMultiEffect(MultiEffectProducer producer) {
		...
		ArrayList<AreaEffectProducer> effectProducers = new ArrayList<>(producer.getContainedEffects().length);
		
		for (EffectProducer effectProducer : producer.getContainedEffects()) {
